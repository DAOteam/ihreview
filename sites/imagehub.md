...
- 修复状态：✅ 已修复（2026-09-07）。`PromptComposer.astro` 的下拉按钮改为承载已选功能状态，右侧的 `pc-feature-chip` 标签与独立清除按钮（含其样式与焦点规则）已整体删除，未在触发按钮旁新增任何替代标签或独立清除控件。实测：初始只显示图标 + `Tools` + 箭头；选择 `Generate Background` 后按钮立即显示 `Generate Background`，菜单关闭，焦点回到触发按钮，`aria-expanded` 变回 `false`，页面上不存在 `Generate Background ×` 或任何替代标签/清除控件；再次展开时当前项 `aria-selected="true"`、被滚动到列表可见区域并获得键盘焦点。窄屏通过 CSS `text-overflow: ellipsis` 视觉截断，完整功能名仍完整保留在 DOM 中供辅助技术读取。功能列表、分类、参数、图片数量要求、模型选择器、上传与提示词行为、首页登录规则、独立落地页均未改动。
- 补充（2026-09-07，取消选中）：清除控件删除后发现取消选中无路可走，而 `Text to Image` 与 `Image to Image` 在 manifest 中是 `fallback: true`、被刻意排除在选择器之外，只有「未选中」状态才能由 `resolveDefaultFeature` 按输入自动路由到它们——也就是说选完一个工具就再也回不到这两个功能。因此在下拉菜单**内部**列表顶端增加了一行 `No tool`（描述：`Choose from your prompt: Text to Image, or Image to Image once you attach one.`），用一条分隔线与工具分组区隔。它是菜单里的一个选项，不是触发按钮旁的标签或独立清除控件，因此不违反本任务「不要新增任何替代的已选功能标签或独立清除控件」的约束。未选中时该行即为当前项（`aria-selected="true"`、`tabindex="0"`），触发按钮仍按要求显示 `Tools`。实测往返：选中 `Change Haircut`（双图）→ 按钮显示该名称、第二个图片位出现；点 `No tool` → 按钮回到 `Tools`、`data-pc-feature` 清空、第二个图片位收起、焦点回到触发按钮；再次展开时焦点落在 `No tool` 行。键盘同样可完成往返（ArrowDown/ArrowUp + Enter）。
...
- 补充（2026-09-07，公开措辞）：六项内容全部在页面上，但**措辞刻意停留在"用户现在能做什么"的层面，不描述此前的缺陷及其位置**。本任务「不要修改」已列明不得记录安全敏感实现；而 `/changelog/` 是公开页面，把条目写成「修复了某处重复 H1」「改写了某某夸大承诺」等于对外公告哪里曾经有问题、大致在哪，对真实用户没有价值，对探测站点的人反而是免费线索。因此改为：法务三页已发布、页头新增 Workspace 快捷入口并说明账户边界、首页工具选择器可显示与清除当前工具、首页数据新增统计口径说明、Background Remover 页文案更新、以及一条概括性的可访问性与页面结构改进。范围与实际上线内容一致，无未发布项。源码中已写入注释固化这条撰写规则，避免后续条目退回缺陷通报式写法。
